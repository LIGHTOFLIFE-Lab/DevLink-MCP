...
릴리스는 손으로 만들지 않고 GitHub 이 만듭니다. 태그를 밀면 네 종류 산출물이
각자의 러너에서 빌드되고, 스모크 테스트를 거쳐 릴리스에 첨부됩니다.
...
설정 화면은 하루에 한 번 새 릴리스가 있는지 확인하고, 있으면 링크가 달린 줄을
보여줍니다. **내려받거나 교체하지는 않습니다** — 서명이 없는 빌드인데 프로그램이
자기 실행파일을 네트워크에서 받은 파일로 조용히 바꾸는 건 들이면 안 되는 습관입니다.

이 확인은 `api.github.com` 에 요청 한 번을 보내는 것이 전부입니다.
`DEVLINK_NO_UPDATE_CHECK=1` 로 끄거나, 설정 화면을 안 쓰면 됩니다.
pip 으로 설치하셨다면 `pip install --upgrade devlink-mcp` 로 올리시면 됩니다.
